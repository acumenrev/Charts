...
open class BarChartView: BarLineChartViewBase, BarChartDataProvider
{
    ...
    fileprivate var _drawValueEnabled = false
    ...
    /// if set to true, all values are drawn above their bars, instead of below their top
    open var drawValueEnabled: Bool
        {
        get { return _drawValueEnabled }
        set
        {
            _drawValueEnabled = newValue
            if(renderer != nil){
                (renderer as! BarChartRenderer).setDrawValueEnabled(enable: newValue)
            }
            setNeedsDisplay()
        }
    }
    ...
    // draw value
    open var isDrawValueEnabled: Bool { return drawValueEnabled }
