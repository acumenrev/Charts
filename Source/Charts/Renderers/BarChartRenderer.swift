...
open class BarChartRenderer: BarLineScatterCandleBubbleRenderer
{
    ...
    var drawValueEnabled = false
    public func setDrawValueEnabled(enable :Bool){
        drawValueEnabled = enable
    }
    ...
    open func drawDataSet(context: CGContext, dataSet: IBarChartDataSet, index: Int)
    {
        ...
        for j in stride(from: 0, to: buffer.rects.count, by: 1)
        {
            ...
            if !isSingleColor {
                ...
                if let e = dataSet.entryForIndex(j) as? HealthKitBarChartDataEntry {
                    if e.isCurrentSelectedBar == true {
    ...
    {
        // TODO: Hai: add option to draw value
        if !drawValueEnabled{
            drawValueBottom(context: context)
            return
        }
    ...
    fileprivate func drawValueBottom(context: CGContext){
        ...
        for dataSetIndex in 0 ..< barData.dataSetCount
        {
            ...
            for j in 0 ..< dataSet.entryCount
            {
                ...
                var point = CGPoint(x: CGFloat(e.x), y: 0)
                    
                // position.y = 0.0
                point = point.applying((trans?.valueToPixelMatrix)!)
                ...
                if let s = e.data{
                    drawValue(
                        context: context,
                        value: String(describing: s),
                        xPos: (point.x),
                        yPos: (point.y),
                        font: dataSet.valueFont,
                        align: .center,
                        color: e.valueTextColor)
                }
            }
