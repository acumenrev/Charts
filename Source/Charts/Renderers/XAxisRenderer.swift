...
{
    open var useSpecialAlignment : Bool = false
    open var ignoreXAxis : Bool = false
    ...
    open func drawLabels(context: CGContext, pos: CGFloat, anchor: CGPoint)
    {
        ...
        for i in stride(from: 0, to: entries.count, by: 1)
        {
            ...
            if viewPortHandler.isInBoundsX(position.x)
            {
                ...
                if let valueFormatter = xAxis.valueFormatter as? MeasurementHistoryLineChartXAxisFormatter {
                    let obj = valueFormatter.axisLabelInfoForValue(xAxis.entries[i], axis: xAxis)
                    anchorToDraw = obj.1
                    label = obj.0
                }
